...
class PR {
    ...
    set config(config) {
        if ("src_file" in config && !Array.isArray(config.src_file)) {
            config.src_file = [config.src_file];
        }
    ...
    async requestIncludes() {
        this.includes = await scanIncludes(Head.fromPR(this).github_repo_url, this.config.src_file[0], this.config.type);
    }
    ...
    getRelevantSrcFiles() {
        return [this.config.src_file[0], ...(this.includes || [])];
    }
