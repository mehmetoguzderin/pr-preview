...
class PR {
    ...
    getPreviewFiles() {
        ...
        this.config.src_file.forEach((s, i) => {
            if (this.processor == "wattsi") {
                this.wattsi.push(new WattsiClient(this));
                values.push(this.wattsi[i].getFilenames().then(_ => {
                    this.wattsi[i].modifiedFiles.forEach(f => {
                        this.preview_files.push(new WattsiFile(this, this.wattsi[i], f));
                    });
                    this.wattsi[i].addedFiles.forEach(f => {
                        this.preview_files.push(new AddedWattsiFile(this, this.wattsi[i], f));
                    });
                    this.wattsi[i].removedFiles.forEach(f => {
                        this.unchanged_files.push(new RemovedWattsiFile(this, this.wattsi[i], f));
                    });
                    this.wattsi[i].unchangedFiles.forEach(f => {
                        this.unchanged_files.push(new UnchangedWattsiFile(this, this.wattsi[i], f));
                    });
                }));
            } else {
                this.preview_files.push(new File(this, s));
    ...
    cacheAll() {
        ...
        return next(this.preview_files.slice(0))
            ...
            .then(_ => {
                if (this.wattsi) { return Promise.all(this.wattsi.map(w => w.cleanup())); }
            });
    }
}
