...
module.exports.validate = function(config) {
    ...
    if (config.src_file && Array.isArray(config.src_file) && config.type.toLowerCase() != "bikeshed") {
        throw TypeError("Invalid config, only bikeshed can have src_file as array");
    }
};
