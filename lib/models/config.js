...
module.exports.validate = function(config) {
    ...
    if (Array.isArray(config.src_file) && config.type.toLowerCase() != "bikeshed") {
        throw Error("Invalid config, src_file can only be array when bikeshed")
    }
};
...
module.exports.addDefault = function(config) {
    ...
    if (config.src_file && !Array.isArray(config.src_file)) {
